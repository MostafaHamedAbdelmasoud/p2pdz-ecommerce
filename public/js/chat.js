...
$(document).ready(function () {
  ...
  window.loadConversations(); // load more conversations
  ...
  window.loadConversation = function (conversation_id, partner_id, source, adminLocation) {
    ...
    if (window.loadingConversationInPRogress == 0) {
      ...
      axios.get('/chat/conversations/' + conversation_id + '/partner/' + partner_id).then(function (response) {
  ...
  window.loadMoreChatReplies = function () {
    if (window.loadMoreRepliesInProgress == 0) {
      ...
      axios.get('/chat/conversations/previous_replies/' + window.active_chat + '/' + top_reply.attr('data-reply-id')).then(function (response) {
        //console.log( JSON.stringify(response.data) );
  ...
  window.loadPartnerNewReplies = function () {
    ...
    if (window.active_chat != 0) {
      ...
      axios.get('/chat/conversations/new_replies/' + window.active_chat + '/' + lastPartnerReply).then(function (response) {
        if (response.data.replies) {
  ...
  loadPartnerNewReplies(); // if there is new partner replies go down by clicking the new replies icon

  $("#new-partner-replies-go-down, #scroll-down-go-down").click(function () {
    $("#new-partner-replies-go-down").removeClass('visible');
    window.activeConversation.scroll({
      top: window.activeConversation.scrollHeight,
      left: 0,
      behavior: "smooth"
    });
  ...
  $("#sub_conversation_scroll-down-go-down").click(function () {
    window.activeSubConversation.scroll({
      top: window.activeSubConversation.scrollHeight,
      left: 0,
      behavior: "smooth"
    });
  });
    ...
    window.Swal.fire({
      ...
      preConfirm: function preConfirm() {
        if (!$("#chat_image_inp").val()) {
          ...
          $("#chat-upload-progress .progress-bar").css({
            width: '0%'
          });
...
module.exports = __webpack_require__(/*! /home/mostafa/Downloads/project mostaql/resources/js/chat.js */"./resources/js/chat.js");
